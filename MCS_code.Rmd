...
Figure 5: GHG Sensitivity analysis
```{r}
# GHG mitigation
src_ghg <- data.frame(p_count =  rowSums(intv_boundary_mgnl[, -1]),
                      ...
                      kwh_ef = rowMeans(ef_sim_electricity[, -1]),
                      ghg_mit = rowSums(ghg_mitigation[, -1]))

inputs <- src_ghg[, 1:(ncol(src_ghg) - 1)]  # Model parameters (adjust column range)
output <- src_ghg[, ncol(src_ghg)]   # The final column is the emissions reduction result

# Fit a linear regression model using inputs to predict the output
lm_model <- lm(output ~ ., data = inputs)

# Standardize the entire dataframe (excluding output)
df_standardized <- src_ghg  
df_standardized[, -ncol(df_standardized)] <- scale(src_ghg[, -ncol(src_ghg)])

# Fit the model on the standardized dataframe
lm_standardized <- lm(scale(output) ~ ., data = df_standardized[, -ncol(df_standardized)])

# Get standardized coefficients (excluding intercept)
standardized_coefs <- coef(lm_standardized)[-1]  

# Step 3: Calculate confidence intervals for unstandardized model
ci_unstandardized <- confint(lm_model)

# Extract standard deviation of predictors and response
sd_x <- apply(src_ghg[, -ncol(src_ghg)], 2, sd)  # Standard deviation of predictors
sd_y <- sd(src_ghg$ghg_mit)  # Standard deviation of response

# Convert confidence intervals to standardized scale
ci_standardized <- ci_unstandardized[-1, ] * (sd_x / sd_y)  # Exclude intercept

# Step 4: Combine results into a data frame
src_df <- data.frame(
  Variable = names(standardized_coefs),
  Coefficient = standardized_coefs,
  CI_Low = ci_standardized[, 1],
  CI_High = ci_standardized[, 2])

p5 <- ggplot(src_df, aes(x = reorder(Variable, Coefficient), y = Coefficient)) +
  geom_point(shape = 18, size = 3, color = "deepskyblue4") +
  ...
  theme_minimal() +
  scale_x_discrete(labels = c("p_count" = "Project count",
                              "ref_mwh" = "Reference kWh\ndemand",
                              "ref_mmbtu" = "Reference therms\ndemand",
                              "ashp_savings" = "Air source heat\npump savings",
                              "weatherize_savings" = "Weatherization\nsavings",
                              "hpwh_savings" = "Heat pump water\nheater savings",
                              "kwh_ef" = "Electricity\nemission factor")) +
  theme(text = element_text(size = 8, color = "black"),
        plot.title = element_text(face = "bold", size = 12, color = "black"),
        axis.text = element_text(size = 8, color = "black"),
...
ggsave("figures/fig5.png", plot = p5, width = 6.5, height = 3, dpi = 300)

```


Figure 6: Capex and Opex
```{r}
# Capex & opex cashflow
...
p6 <- p_capex + p_opex + p_cashflow

ggsave("figures/fig6.png", plot = p6, width = 6.5, height = 2, dpi = 300)

```

Figure 7: 6 panel CBA indicators
```{r}
...
cba_econ_plot("npv_econ_plot", npv_stats, c = "deepskyblue4", ylab = "Economic Boundary\n \nNPV ($1,000 USD)", xlab = NULL, panel = "a")
cba_social_plot("npv_social_plot", npv_stats, c = "deepskyblue4", ylab = "Social Boundary\n \nNPV ($1,000 USD)", xlab = expression("r"[e]), panel = "d")
cba_econ_plot("sroi_econ_plot", sroi_stats, c = "palegreen4", ylab = "SROI (benefits:costs)", xlab = NULL, panel = "b")
cba_social_plot("sroi_social_plot", sroi_stats, c = "palegreen4", ylab = "SROI (benefits:costs)", xlab = expression("r"[e]), panel = "e")
cba_econ_plot("lcca_econ_plot", lcca_stats, c = "orangered4", ylab = "LCCA (USD/tco2e)", xlab = NULL, panel = "c")
cba_social_plot("lcca_social_plot", lcca_stats, c = "orangered4", ylab = "LCCA (USD/tco2e)", xlab = expression("r"[e]), panel = "f")

p7 <- npv_econ_plot + sroi_econ_plot + lcca_econ_plot + npv_social_plot + sroi_social_plot + lcca_social_plot + 
  plot_layout(ncol = 3, nrow = 2)

ggsave("figures/fig7.png", plot = p7, width = 6.5, height = 4.8, dpi = 300)

```

Extra figure: CBA Sensitivity analysis
```{r}
# GHG mitigation
dr_count = dr_ub*10 - dr_lb*10 + 1
npv_long <- npv_econ_dr %>%
  pivot_longer(cols = everything(), 
               names_to = "dr",
               values_to = "NPV")
ghg_mit_long <- data.frame(rep(rowSums(ghg_mitigation[, -1]), times = dr_count))
capex_long <- data.frame(rep(rowSums(cashflow_investment[, -1]), times = dr_count))
opex_long <- data.frame(rep(rowSums(cashflow_opex[, -1]), times = dr_count))

src_npv <- data.frame(ghg_mit = unname(ghg_mit_long),
                      capex = unname(capex_long),
                      opex = unname(opex_long),
                      dr = unname(npv_long$dr),
                      npv = unname(npv_long$NPV))

src_npv$dr <- as.numeric(src_npv$dr)

inputs2 <- src_npv[, 1:(ncol(src_npv) - 1)]  # Model parameters (adjust column range)
output2 <- src_npv[, ncol(src_npv)]   # The final column is the emissions reduction result

# Fit a linear regression model using inputs to predict the output
lm_model2 <- lm(output2 ~ ., data = inputs2)

# Standardize the entire dataframe (excluding output)
df_standardized2 <- src_npv  
df_standardized2[, -ncol(df_standardized2)] <- scale(src_npv[, -ncol(src_npv)])

# Fit the model on the standardized dataframe
lm_standardized2 <- lm(scale(output2) ~ ., data = df_standardized2[, -ncol(df_standardized2)])

# Get standardized coefficients (excluding intercept)
standardized_coefs2 <- coef(lm_standardized2)[-1]  

# Step 3: Calculate confidence intervals for unstandardized model
ci_unstandardized2 <- confint(lm_model2)

# Extract standard deviation of predictors and response
sd_x2 <- apply(src_npv[, -ncol(src_npv)], 2, sd)  # Standard deviation of predictors
sd_y2 <- sd(src_npv$npv)  # Standard deviation of response

# Convert confidence intervals to standardized scale
ci_standardized2 <- ci_unstandardized2[-1, ] * (sd_x2 / sd_y2)  # Exclude intercept

# Step 4: Combine results into a data frame
src_df2 <- data.frame(
  Variable = names(standardized_coefs2),
  Coefficient = standardized_coefs2,
  CI_Low = ci_standardized2[, 1],
  CI_High = ci_standardized2[, 2])

p8 <- ggplot(src_df2, aes(x = reorder(Variable, Coefficient), y = Coefficient)) +
  geom_point(shape = 17, size = 2, color = "deepskyblue4") +
  geom_errorbar(aes(ymin = CI_Low, ymax = CI_High), width = 0.2, color = "black") +
  coord_flip() +  # Flip to horizontal view
  labs(title = NULL,
       x = "Model Parameters",
       y = "Standardized Coefficient") +
  theme_minimal() +
  scale_x_discrete(labels = c("ghg_mit" = "Mitigation outcome",
                              "capex" = "Capex",
                              "opex" = "Opex",
                              "dr" = "Economic discount rate")) +
  theme(text = element_text(size = 8, color = "black"),
        plot.title = element_text(face = "bold", size = 12, color = "black"),
        axis.text = element_text(size = 8, color = "black"),
        axis.title = element_text(size = 8, color = "black"),
...
ashp_outputlist <- (list("ashp_boundary_mgnl" = ashp_boundary_mgnl,
                         ...
                         "ashp_intv_factor" = ashp_intv_factor,
...
weatherize_outputlist <- (list("weatherize_boundary_mgnl" = weatherize_boundary_mgnl,
                         ...
                         "weatherize_intv_factor" = weatherize_intv_factor,
...
hpwh_outputlist <- (list("hpwh_boundary_mgnl" = hpwh_boundary_mgnl,
                         ...
                         "hpwh_intv_factor" = hpwh_intv_factor,
